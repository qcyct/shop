<template>
	<view class="notice bottom-cell-group" v-if="noticeData.noticeList.length > 0">
		...
		<swiper class="notice-c" :indicator-dots="false" :autoplay="true" :interval="3000" :duration="1000" :vertical="true"
		 :circular="true">
			<swiper-item v-for="(item,index) in noticeData.noticeList" :key="index">
				<view class="swiper-item" @click="goNotice(item)">{{ item.title }}</view>
			</swiper-item>
		</swiper>
	</view>
</template>
