		...
		<view class="list">
			...
			<view class="sub-list valid" :class="subState">
				...
				<view class="row" v-for="(row,index) in couponValidList" :key="index">
					<!-- 删除按钮 -->
					<view class="menu" @tap.stop="deleteCoupon(row.couponId,couponValidList,index)">
					...
					<view class="carrier" :class="[typeClass=='valid'?theIndex==index?'open':oldIndex==index?'close':'':'']"
					 @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
						<CouponItme :CouponListItem="row" :stateType="2" :CouponClick="CouponClick"></CouponItme>
					</view>
				</view>
			</view>
			<view class="sub-list invalid" :class="subState">
				...
				<view class="row" v-for="(row,index) in couponinvalidList" :key="index">
					<!-- 删除按钮 -->
					<view class="menu" @tap.stop="deleteCoupon(row.couponId,couponinvalidList,index)">
						<view class="icon shanchu"></view>
					</view>
					<!-- content -->
					<view class="carrier" :class="[typeClass=='invalid'?theIndex==index?'open':oldIndex==index?'close':'':'']"
					 @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
						<CouponItmeEnd :CouponListItem="row"></CouponItmeEnd>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	import CouponItmeEnd from '../../components/CouponItmeEnd/CouponItmeEnd.vue';
	export default {
		components: {
			CouponItme,
			CouponItmeEnd
		...
		methods: {
			...
			//获取优惠卷信息
			getCouponByUser() {
				this.$api.getCouponByUser({}, (res) => {
					if (res.code) {
						this.couponValidList = res.data.couponValidList;
						this.couponinvalidList = res.data.couponinvalidList;
					}
				})
			},
			CouponClick(goodsId) {
